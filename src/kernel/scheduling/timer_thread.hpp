...
//
// PHASE 0: declaration only. The implementation lands with the timer-thread
// unit; the pimpl is what lets that unit work entirely inside the .cpp.
...
namespace grab::kernel::scheduling
{
    ...
    class TimerThread final
    {
        public:
            ...
            // Readable when at least one armed deadline has expired.
            [[nodiscard]]
            int
            wake_fd() const noexcept;

            // The tokens whose deadlines have passed, removed from the timer
            // set. Empty is the normal answer.
            [[nodiscard]]
            std::vector<Token>
            drain();
