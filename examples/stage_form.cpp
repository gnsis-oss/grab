...
namespace
{
    ...
    {
        return "left:" +
               std::to_string( static_cast<int>( rect.x_ ) ) +
               "px;top:" +
               std::to_string( static_cast<int>( rect.y_ ) ) +
               "px;width:" +
               std::to_string( static_cast<int>( rect.w_ ) ) +
               "px;height:" +
               std::to_string( static_cast<int>( rect.h_ ) ) +
               "px;";
    }
    ...
    page_html()
    {
        ...
        constexpr const char* pixel_gif = "data:image/gif;base64,R0lGODlhAQABAIAAAP///"
                                          "wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==";
        const std::string     catalog_types[] = {
            "button",
            "color",
            "datetime-local",
            "email",
        ...
        for( const std::string& type : catalog_types )
        {
            ...
            catalog += " style=\"position:absolute;left:" +
                       std::to_string( left ) +
                       "px;top:" +
                       std::to_string( top ) +
        ...
        return std::string{
               ...
               px_rect( pass_rect ) +
               "\">\n"
               "<input type=\"submit\" id=\"f_apply\" aria-label=\"" +
               apply_idle +
               "\" value=\"" +
               apply_idle +
               "\" style=\"position:absolute;" +
    ...
    build_scene()
    {
        ...
        expect( "checkbox_mirrored", stage::Observe::A11yName, "m_check", "m_check on" );
        expect( "radio_mirrored", stage::Observe::A11yName, "m_radio", "m_radio b" );
        expect( "combo_mirrored", stage::Observe::A11yName, "m_combo", "m_combo beta" );
        ...
        expect( "password_length_mirrored",
                stage::Observe::A11yName,
                "m_pass",
                "m_pass 7" );
        expect( "apply_reports_applied", stage::Observe::A11yName, "apply", apply_done );
      ...
      char** argv )
{
    ...
    std::cout << "AUTHOR\n  form      9 driven controls + a 22-type catalog -> " << page
              << '\n';
    ...
    int exit_code = 1;
    {
        ...
        std::cout << "  calibrate scale (" << scale_x << "," << scale_y << ")  origin ("
        ...
        if( calibration_sane )
        {
            ...
            else
            {
                ...
                std::cout << "  verify    second anchor off by "
                          ...
                          << " px (<= " << static_cast<int>( anchor_slack_px )
                          << " arms the run)\n";
            ...
            [&]( const view::ViewRect& target, const view::ViewRect& aim )
        {
            ...
            motion::Vec2 cursor{
                at.has_value() ? static_cast<double>( at->x ) : target.x_,
                at.has_value() ? static_cast<double>( at->y ) : target.y_
            };
        ...
        ( void )( *input ).move(
            static_cast<std::int16_t>( window_rect.x_ + ( window_rect.w_ * park_fx ) ),
            static_cast<std::int16_t>( window_rect.y_ + ( window_rect.h_ * park_fy ) )
        );
        ...
        for( int attempt = 0; attempt < 10 && !option.has_value(); ++attempt )
        {
            if( auto synced = ( *session )->resync(); synced.has_value() )
            {
                ...
                {
                    auto matches =
                        ( *session )->resolve_all( grab::sel::role( role_id ) );
                    if( !matches.has_value() )
                    {
                        continue;
                    }
                    for( const grab::Match& match : *matches )
                    {
                            ...
                            info.bounds.w <= 500.0 )
                        {
                            option = Live{
                                .name_ = info.name,
                                .rect_ =
                                    ladder::view::fid::current().rect( view::ViewRect{
                                        .x_ = info.bounds.x,
                                        .y_ = info.bounds.y,
                                        .w_ = info.bounds.w,
                                        .h_ = info.bounds.h
        ...
        if( option.has_value() )
        {
            std::cout << "  combo     option \"beta\" at ("
                      ...
                      << static_cast<int>( option->rect_.h_ ) << ") — clicking it\n";
            const view::ViewRect aim{
                .x_ = option->rect_.center_x() - 4.0,
        ...
        std::cout << "  password  click, type "
                  << std::string_view{ typed_secret }.size() << " characters\n";
        ...
        // ── 5. READ THE MIRRORS ─────────────────────────────────────────────
        const auto mirror_value = [&]( std::string_view id ) -> std::string
        {
        ...
        const std::array<const char*, 7U> mirror_ids{
            "m_text",
            "m_check",
            "m_radio",
            "m_combo",
        ...
        scene.expect_.push_back( stage::Expectation{
            ...
            .observe_   = stage::Observe::A11yName,
            .subject_   = "receipt",
            .value_     = std::string{ "SUMMARY text=" } +
                          typed_text +
                          " check=on" +
                          " radio=b combo=beta range=" +
                          range_value +
                          " number=" +
                          typed_number +
                          " date=" +
        ...
        {
            if( auto again = resolve_prefix( **session, grab::role::button, "APPL" );
                again.has_value() )
            {
