...
| 08 | `suite/08_stage_form.sh` → `stage_form.cpp` | The **controls**: a form carrying **every HTML input type — all 22** — plus a combo box. Driven for real: text, checkbox, radio, combo (click, Escape, then Down — by keyboard, deliberately), a **range slider** clicked at 80% of its track (asserted as a band, never an exact number), number, date, password, and a real `type=submit`. Aiming is by authored geometry plus chrome measured from the APPLY button (exotic input types have no stable a11y role); every value is verified through page-side mirrors read over a11y, the APPLY summary of what the page's script received, and a catalog element enumerating the DOM's input types — which must be all 22. 16-check scorecard. |
