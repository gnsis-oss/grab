...
> **Correction (2026-07-17, after Wave-6):** `pid.hpp` **stays too** (migration row 10 deferred, not done). `grab::Pid` is a semantic *serializable* pid (unknown = 0 → `""`), distinct from `BorrowedProcessId` (unknown = -1, pidfd-ownership identity), and it is baked into the public `Event` vocabulary (`event.hpp` `WindowChange::pid`), the `eventgrab.v1` wire, and JSONL storage. Swapping the type would change the wire/storage format — a deliberate v1-round-trip-gated change, not a mechanical relocation. Retire it later as its own TDD task if ever wanted; for now it is a valid public header. `payload_fields.hpp` likewise stays (internalizing it would touch the public event vocabulary).
>
