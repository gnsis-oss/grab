...
namespace
{
          ...
          BareXvfbRejectsOverlayWithoutMappingAWindow )
    {
        ...
        // root_children() is a reply round trip: once it returns, the server
        // has processed the mask change, so the subscription is active for
        ...
        // Synchronize (reply round trip) so every event generated by the
        // attempts has been delivered, THEN drain the queue: no MapNotify may
        // have arrived at any point during the attempts.
        const auto  after             = root_children( display );
          ...
          PublicOverlayTrailAndInputComposeWithClickThrough )
    {
        ...
        // flush() fences the delegate + X server, but the external
        // compositor's repaint is asynchronous and unfenced (spec §4):
        // bounded-retry the capture until the annotation is composited.
        constexpr int captureAttempts = 50;
        bool          annotation_seen = false;
        bool          trail_seen      = false;
        for( int attempt = 0; attempt < captureAttempts; ++attempt )
        {
            ...
            const auto annotation =
                pixel_at( *image, annotationProbeX, annotationProbeY );
            ASSERT_TRUE( annotation.has_value() );
            annotation_seen = is_green( *annotation );
            trail_seen      = has_blue_near_trail_probe( *image );
            if( annotation_seen && trail_seen )
            {
                break;
            }
            ASSERT_TRUE( ( *overlay )->flush().has_value() );
        }
        EXPECT_TRUE( annotation_seen );
        EXPECT_TRUE( trail_seen );
